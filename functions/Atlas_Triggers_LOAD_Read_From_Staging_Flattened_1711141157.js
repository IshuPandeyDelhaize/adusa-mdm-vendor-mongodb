...
This third Atlas trigger will read documents in the “flattened-docs” database and will do the following:
		If the flattened document is for the BP, then:
		Write new flattened document based on the BP into the Versioned Audit database with a subset of attributes by 
...
function formatDateForMongo(date) {
  ...
  const targetCollection = context.services.get("VMDM-DEVCluster").db("vmdm-versioned-Audit").collection("entity");  
   ...
   const query = {objectTypeID: "BusinessPartner" };
	// Log the count of documents to process
	const countToProcess = await flattenedCollection.count(query);
	console.log(`Count of documents to process: ${countToProcess}`);
	
	 // Iterate through each BP  document and process it
	 const docsFromFlattenedCollection = await flattenedCollection.find(query).toArray(); 
 
for (const sourceDoc of docsFromFlattenedCollection) {
	  const sourceDocID = sourceDoc._id;
  ...
  try{
			...
			console.log("sourceDoc.stepID: ", sourceDoc.stepID);
			console.log("sourceDocID: ", sourceDocID);
...
if(accountsPayableRef){
			...
			// do a lookup for the AP document in the Staging.flattened collection to get values from it.
								...
								// do a lookup for the legacy AP document in the Staging.flattened collection to get values from it.
									for (const legacyAPRefID of legacyAPdocList) {
										console.log("In loop counter ID from idList:",legacyAPRefID);
											...
											if (legacyAPdoc) {
												console.log(`Document with stepID:  '${legacyAPRefID}' found.`);
												...
												// Next line is if no legacyAP doc was found
													...
													console.log(`Document with stepID '${legacyAPRefID}' not found.`);
									}
									} // end for loop
			  ...
			  if(apAddrRefArray){
				  apAddressList = apAddrRefArray.split(';').map(id => id.trim());
				  for (const apAddrRefID of apAddressList) {
					const apAddrDoc = await flattenedCollection.findOne({ "stepID": apAddrRefID });
					if(apAddrDoc){
						const apAddrStatus = apAddrDoc['values.a_AddressStatus'];
						if(apAddrStatus === "CURRENT"){
		  ...
		  if (apContactRefArray){
				  ...
				  for (const apContactRefID of apContactList) {
					  ...
					  if (apContactDoc) {
						   console.log(`Document with stepID '${apContactRefID}' found.`);
					   ...
					   if(apContactList.slice(-1).toString() !== apContactRefID){
							...
							console.log(`Document with stepID '${apContactRefID}' not found.`);
					  }
					 } // end for loop
			...
			if (bpChangeLogRefArray){
							...
							for (const changeLogRefID of bpChangeLogList) {
									...
									if (changeLogDoc) {
											console.log(`Document with stepID '${changeLogRefID}'  found.`);
									...
									console.log(`Document with stepID '${changeLogRefID}' not found.`);
							  }
							} // end for loop
...
console.log(`Before lastChangeLogRef = values[0];`);
						if(keyValueArray.length >0){
						  console.log(`Inside 	if(keyValueArray.length >0), Before lastChangeLogRef = values[0];`);
...
console.log("After if bpChangeLofRefArray lastChangeLogRef = ", lastChangeLogRef);		
			...
			if (bpAuditRefArray){
				  ...
				  if (bpAddressRefArray){
					  const bpAddressList = bpAddressRefArray.split(';').map(id => id.trim());
					 ...
					 for (const addrRefID of bpAddressList) {
						  ...
						  if(addrDoc){
							const bpAddrStatus = addrDoc['values.a_AddressStatus'];
							if(bpAddrStatus === "CURRENT"){
				  ...
				  if (bpContactRefArray){
							...
							for (const contactRefID of bpContactList) {
									...
									if (bpContactDoc) {
											console.log(`Document with stepID '${contactRefID}' found.`);
											...
											console.log(`Document with stepID '${contactRefID}' not found.`);
										}
							} // end for loop
					...
					const versionedDocument = {
						...
						//	Maintenance_Date: sourceDoc['values.a_DateLastPublished'],    ToDO change back after the approve BP is AFTER setting Date in Sandbox
							Maintenance_Date: formatTimeStampForMongo(new Date()),
